...
var GitHubClient = class {
  ...
  async prDetail(repoFullName, num) {
    const res = await this.get(
  ...
  async prChecksState(repoFullName, headSha) {
    if (!headSha) return { ok: true, data: "none" };
    const res = await this.get(
      `/repos/${repoFullName}/commits/${headSha}/check-runs`
    );
    if (!res.ok) return res;
    return { ok: true, data: rollupChecks(res.data.check_runs ?? []) };
  }
  async prReviewState(repoFullName, num) {
    const res = await this.get(
      `/repos/${repoFullName}/pulls/${num}/reviews`
    );
    if (!res.ok) return res;
    return { ok: true, data: deriveReviewState(res.data ?? []) };
  }
};
var BAD_CONCLUSIONS = /* @__PURE__ */ new Set([
  "failure",
  "timed_out",
  "cancelled",
  "action_required",
...
function deriveReviewState(reviews) {
...
var VaultWriter = class {
  ...
  async writePage(path2, content) {
    ...
    try {
      ...
      return { ok: true, data: { path: json.path ?? path2 } };
    } catch (err) {
...
function classifyPr(facts, now, staleDays) {
  const buckets = [];
  ...
  if (facts.reviewState === "approved" && facts.checksState === "success" && (facts.mergeableState === "clean" || facts.mergeableState === "unstable")) {
    buckets.push("ready-to-merge");
  }
  const updated = Date.parse(facts.updatedAt);
  if (!Number.isNaN(updated)) {
    const ageDays2 = (now.getTime() - updated) / 864e5;
    if (ageDays2 >= staleDays) buckets.push("stale");
  }
  return buckets;
}
...
function ageDays(updatedAt, now) {
  ...
  return Number.isNaN(t) ? -1 : Math.floor((now.getTime() - t) / 864e5);
}
function renderDigest(assessed, generatedAt, errors) {
  const lines = [
    "---",
    ...
    "",
    "## \u{1F514} Needs your attention",
    ""
  ];
  const attention = assessed.filter(
    (a) => a.buckets.some((b) => ATTENTION_BUCKETS.includes(b))
  );
  if (attention.length === 0) {
    lines.push("- Nothing flagged. \u{1F389}");
  } else {
    const rank = (a) => {
...
// src/job.ts
async function runPrTriage(deps) {
...
async function build(ctx) {
  ...
  return {
    client: new GitHubClient({ token: cfg.githubToken, org: cfg.org, timeoutMs: 15e3 }),
    writer: new VaultWriter({ baseUrl: cfg.vaultServerUrl, timeoutMs: 1e4 }),
    cfg
  };
}
function ok(data) {
