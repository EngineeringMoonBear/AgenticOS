...
var VaultClient = class {
  ...
  async discardInboxItem(inboxPath) {
    let decoded;
    try {
...
// src/tools/read.ts
async function handleRead(client, input) {
...
// src/tools/stats.ts
async function handleStats(client) {
  const result = await client.getStats();
  if (!result.ok) return { error: result.error };
  return { ...result.data };
}

// src/actions/discard.ts
async function handleDiscard(client, input) {
...
var plugin = definePlugin({
  async setup(ctx) {
    ...
    const client = new VaultClient({ baseUrl, timeoutMs: 5e3 });
    ...
    ctx.tools.register(
      ...
      async (params) => toToolResult(
        await handleSearch(client, params)
      )
    );
    ctx.tools.register(
      "vault_read",
      ...
      async (params) => toToolResult(await handleRead(client, params))
    );
    ctx.tools.register(
      "vault_list",
      {
    ...
    ctx.actions.register("vault_discard", async (params) => {
      const path2 = String(params.path ?? "");
      if (!path2) throw new Error("path is required");
      return handleDiscard(client, { path: path2 });
    });
