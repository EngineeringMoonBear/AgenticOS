...
function verifyKeepSignature(rawBody, secret, signatureHeader) {
...
var SEVERITIES = ["critical", "high", "warning", "info", "low"];
var DEFAULT_MINT_SEVERITIES = ["critical", "high", "warning"];
function normalizeSeverity(raw) {
  const s = typeof raw === "string" ? raw.toLowerCase().trim() : "";
  return SEVERITIES.includes(s) ? s : "warning";
}
...
function parseKeepAlert(raw) {
  if (!raw || typeof raw !== "object") return null;
  const o = raw;
  const fingerprint = typeof o.fingerprint === "string" ? o.fingerprint.trim() : "";
  const name = typeof o.name === "string" && o.name.trim() ? o.name.trim() : typeof o.title === "string" ? o.title.trim() : "";
  if (!fingerprint || !name) return null;
  const status = typeof o.status === "string" ? o.status.toLowerCase().trim() : "firing";
  const url = typeof o.url === "string" && o.url ? o.url : typeof o.generatorURL === "string" ? o.generatorURL : "";
  return {
    fingerprint,
    name,
...
function shouldMint(sev, mintSeverities) {
  return mintSeverities.includes(sev);
}
function routingTokens(alert) {
  const parts = [
    alert.name,
    alert.service,
    alert.environment,
...
function resolveOwnership(alert, rules) {
...
function keepMarker(fingerprint) {
  return `<!-- keep-alert-fingerprint: ${fingerprint} -->`;
}
function buildIssueDescription(alert) {
  const lines = [
...
function buildRefireComment(alert, occurrence) {
  const detail = alert.description ? `

${alert.description}` : "";
  const link = alert.url ? ` ([alert](${alert.url}))` : "";
  return `\u{1F501} **Keep alert re-fired** \u2014 occurrence #${occurrence} (severity: ${alert.severity}, status: ${alert.status})${link}.${detail}`;
}
function buildRecurrenceComment(alert, occurrence) {
  const link = alert.url ? ` ([alert](${alert.url}))` : "";
  return `\u26A0\uFE0F **Keep alert recurred after resolution** \u2014 reopened, occurrence #${occurrence} (severity: ${alert.severity})${link}. The condition Keep previously marked resolved is firing again.`;
}
function buildResolutionComment(alert, occurrences) {
  const link = alert.url ? ` ([alert](${alert.url}))` : "";
  return `\u2705 **Keep alert resolved**${link}. Closing this issue after ${occurrences} occurrence${occurrences === 1 ? "" : "s"}. Keep will re-open it (new comment) if the alert fires again.`;
}
function buildOpsPing(info) {
  ...
  switch (info.kind) {
    case "created":
      return `\u{1F6A8} Keep \u2192 Paperclip issue minted: **${info.name}** (${info.severity}) ${who}${route} \xB7 issue \`${info.issueId}\``;
    case "refired":
      return `\u{1F501} Keep alert re-fired: **${info.name}** (${info.severity}) \xB7 issue \`${info.issueId}\``;
    case "recurred":
      return `\u26A0\uFE0F Keep alert recurred after resolution \u2014 reopened: **${info.name}** (${info.severity}) \xB7 issue \`${info.issueId}\``;
    case "resolved":
      return `\u2705 Keep alert resolved \u2014 closed: **${info.name}** \xB7 issue \`${info.issueId}\``;
    case "skipped":
      return `\u{1F515} Keep alert below mint gate (Discord-only): **${info.name}** (${info.severity})${info.reason ? ` \u2014 ${info.reason}` : ""}`;
  }
}

// src/mapping.ts
var MAPPING_TABLE = "keep_alert_mapping";
...
async function getByFingerprint(db, fingerprint) {
...
async function insertNew(db, row) {
  await db.execute(
...
async function updateState(db, fingerprint, patch) {
  await db.execute(
...
// src/worker.ts
var KEEP_ENDPOINT_KEY = "keep-alert";
...
function readConfig(raw) {
  ...
  const mintSeverities = rawMint.map((s) => typeof s === "string" ? s.toLowerCase().trim() : "").filter((s) => ["critical", "high", "warning", "info", "low"].includes(s));
  const rawOwnership = Array.isArray(raw.ownership) ? raw.ownership : [];
  const ownership = rawOwnership.map((r) => {
    const o = r ?? {};
    return {
      match: String(o.match ?? ""),
      assigneeAgentId: String(o.assigneeAgentId ?? ""),
      projectId: o.projectId ? String(o.projectId) : void 0
    };
  }).filter((r) => r.match && r.assigneeAgentId);
  return {
    companyId: raw.companyId ? String(raw.companyId) : void 0,
    projectId: raw.projectId ? String(raw.projectId) : void 0,
    keepWebhookSecret: raw.keepWebhookSecret ? String(raw.keepWebhookSecret) : void 0,
    defaultAssigneeAgentId: raw.defaultAssigneeAgentId ? String(raw.defaultAssigneeAgentId) : void 0,
...
async function mintIssue(ctx, cfg, alert, nowIso) {
...
async function handleFiringExisting(ctx, cfg, alert, nowIso, existing) {
...
async function handleResolution(ctx, cfg, alert, nowIso) {
...
async function handleKeepAlert(ctx, cfg, input) {
