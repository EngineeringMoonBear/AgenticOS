...
>
> **Amendment 2026-07-14 — delegate the agent-runtime secret framework to Paperclip native (#8825); the broker stays the 1Password-backed issuance engine underneath it.** The Paperclip v2026.707.0 upgrade (`Paperclip-AgenticOS` fork; vault `sources/2026-07-14-paperclip-v707-upgrade.md`) shipped a full native secrets framework: per-**user** secret definitions bound to env-keys and **environments**, `responsible_user_id` stamped on every issue/heartbeat_run/routine/routine_run with a pre-dispatch "the responsible human supplied the needed value" gate, a `secret_access_events` audit log, and a **pluggable provider registry** (`server/src/secrets/`; `local_encrypted` default + `aws_secrets_manager`, `gcp_secret_manager`, `kubernetes`, `github`, `external`). This ADR's original plan would have *rebuilt* the user-scoping / environments / attribution / gating / audit layer in the dashboard+broker. **Decision: stop building that layer — adopt Paperclip native for it — and keep this ADR's broker as the secret-*origin* only.**
>
> - **1Password remains the source of truth (unchanged; the D1 no-store-in-app thesis is preserved).** Paperclip's provider contract stores a **reference** in `company_secret_versions.material` (an `op://…` ref) plus an irreversible `valueSha256`; the value lives solely in 1Password and is fetched **live on each `resolveVersion`** — never at rest in Paperclip's DB. Proven by the shipped AWS provider (it stores the ARN; `getSecretValue` fetches live). The only in-flight exposure is transient process memory at run-time — identical to the broker resolving directly, and governed by the `sanitizeRuntimeServiceBaseEnv` fork patch.
> - **Integration = a `onepassword_broker` secret provider** implementing `SecretProviderModule` (~5 methods): `linkExternalSecret({externalRef: "op://…"})` registers the ref (no value stored); `resolveVersion` calls the broker → 1Password → returns the live value. The provider registry (`provider-registry.ts`) and the `SecretProvider` union in `@paperclipai/shared` are **hardcoded**, so this is a **3rd fork patch** (a never-conflicting new file + two one-line re-adds per upstream sync — the same cheap conflict class as the CLI-esbuild patch). Optional long-game: upstream a PR adding plugin-registrable secret providers to make it a zero-patch plugin.
> - **Responsible-user attribution: adopted outright** — already live in the schema (the migration ran), costs no fork patches, and back-fills the human-accountability primitive that the group-coding D5 (multi-human delegation) and D7 (attributable autonomy / P1 restore lane) designs both assumed but never had. Enforced-in-schema now even though there is one human operator today, so multi-human never requires a retrofit.
> - **Net scope change:** the broker's remit **shrinks** to what only it can do — hold the 1Password SA token(s) + backing cloud creds, enforce policy, and issue ephemeral scoped credentials — for its CI/deploy consumers (odoocker, grove-sites) **and** as Paperclip's `onepassword_broker` provider backend. The parallel per-user / attribution / environments subsystem this ADR implied is **removed from scope** (upstream owns it). Sequencing: broker-first (it is the shared backend), the provider second.
